...
export class OrderService {
  private static readonly SHIPPING_COST = 30;
  ...
  static async getCheckoutSummary(userId: string): Promise<CheckoutSummary> {
    ...
    const shippingCost =
      cart.items.length > 0 && cart.subtotal < this.FREE_SHIPPING_THRESHOLD
        ? this.SHIPPING_COST
        : 0;
  ...
  static async placeOrder(
    ...
    // 6. Calculate totals
    const shippingCost =
      cart.subtotal >= this.FREE_SHIPPING_THRESHOLD ? 0 : this.SHIPPING_COST;
    const total = cart.subtotal + shippingCost - cart.discount;
